...
from dataset import RepertoireDataset
...
def get_standard_scaler(config, cache_path):
    ...
    assert (
        config["validation_mode"] == "train_val_split",
    ...
    for bf in config["bag_features"]:
        bag_features_categories[bf] = metadata_df[bf].unique().tolist().index
    categorical_df = to_categorical(metadata_df, bag_features_categories)
    # Standardize feature values
    categorical_scaler = StandardScaler()
    categorical_scaler.fit(categorical_df[config["bag_features"]])
    data_dfs = []
